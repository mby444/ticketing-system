...
- **Dashboard partitions tickets by assignee** (since 2026-09-26): `app/dashboard/page.tsx` splits one `getAllTickets()` result into an **"Assigned to me"** section (active statuses before `Closed`, then `createdAt` desc) and **"Other tickets"** (the rest, original order) — partition, no duplicated rows; count badges + subtitle show both numbers, empty sections render a dashed empty-state note instead of a table. Shared table markup: `components/dashboard/TicketTable.tsx` (server component wrapping `StatusSelect`/`AssignSelect`). Rows move between sections automatically because `updateTicketStatus`/`assignTicket` already call `revalidatePath("/dashboard")`. Verified 2026-09-26 in the browser: partition counts (1+54, 2+53, 0+55 of 55 tickets), Open-before-Closed ordering, assign → moves in / unassign → moves out, empty note, and CLIENT `/dashboard` → `/tickets` redirect. Testing left the DB at its documented baseline: **all 55 tickets unassigned** (it also cleared one stale assignment to user2 left over from the earlier RBAC matrix).
